 ...
 *     an embedded path), satisfying T-SPAE AC-1 (serves with no external files);
 *  2. dev fallback: the `nuxt generate` output on disk (`ui/.output/public`),
 *     so `bun run src/bin.ts daemon` works straight after a UI build without
 *     regenerating the manifest.
 *
 * Any extension-less GET that misses falls back to `index.html` — SPA routes
 * like `/vault/vault-docs` are client-side.
 */
...
/**
 * Serve `pathname` from the embedded manifest or the disk build, `null` when the
 * SPA has nothing for it (the server then 404s). `/` and unknown extension-less
 * paths resolve to `index.html`.
 */
export function serveStatic(pathname: string): Response | null {
  const wantsHtml = pathname === "/" || !/\.[a-z0-9]+$/i.test(pathname);
  return Object.keys(assets).length > 0
    ? serveEmbedded(pathname, wantsHtml)
    : serveDisk(pathname, wantsHtml);
}

/** Resolve from the embedded manifest (compiled binary, or a dev run after gen-assets). */
function serveEmbedded(pathname: string, wantsHtml: boolean): Response | null {
  const hit =
    assets[pathname] ??
    assets[`${pathname.replace(/\/$/, "")}/index.html`] ??
    (wantsHtml ? assets["/index.html"] : undefined);
  return hit ? fileResponse(hit, wantsHtml ? "/index.html" : pathname) : null;
}

/**
 * Disk fallback (dev) — resolve to an actual FILE: the exact path, that path's
 * index.html (Nuxt prerenders routes as directories), or the SPA shell.
 */
function serveDisk(pathname: string, wantsHtml: boolean): Response | null {
  const root = diskRoot();
  if (!root) return null;
  ...
  const candidates = [
    ...
    ...(wantsHtml ? [join(root, "index.html")] : []),
  ];
