/**
 * THE API SEAM — the proposed stable JSON API the local vault-dashboard daemon
 * will later emit, mirroring the route sketch in
 * [[D-0012-distribution-single-exec-and-web-ui]] §D3.
 *
 * These are TYPES ONLY (no runtime values). The prototype binds its mock
 * fixtures to exactly these shapes today; when the real daemon (`apps/web`, a
 * `runner`/`declarative` consumer) lands, its handlers emit the same shapes and
 * the mock loader is swapped for a real `fetch` client with no change to the
 * components.
 *
 * The engine-output shapes below are COPIED from the engine, never imported, so
 * the prototype stays fully decoupled from `src/` (there is intentionally no
 * import edge). If the engine's public shape changes, these are updated by hand:
 *
 *   mirrors src/core/types.ts     → FindingLevel, SourcePos, Finding
 *   mirrors src/runner/corpus.ts  → RunStats, runCorpus() return (RunResult)
 *   mirrors src/declarative/infer.ts → init --check drift semantics (DriftResult)
 *
 * Route map (D-0012 §D3, lightly expanded with a single-vault detail route):
 ...
 * from the committed contract. Mirrors what `inferConfig` would re-derive vs. the
 * on-disk config (section/field/order changes, unknown-admission changes).
 */
 ...
 * drifted; `warnings` carries advisory inference diagnostics (e.g. merged
 * variant headings) that don't themselves constitute drift.
 */
