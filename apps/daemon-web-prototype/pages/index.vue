 ...
 * every kit component is imported EXPLICITLY, the vault list is taken via a prop
 * (stories control it) and only falls back to the `mockApi` seam at runtime, and
 * detail links are plain `<a>` anchors, never `<NuxtLink>`.
 */
...
/**
 * The prop-or-load seam (mirrors `[id].vue`): prefer the `vaults` prop (Storybook),
 * else pull the full mock list at runtime. Spread into a fresh array so a refresh
 * re-assigns a new reference and the reactive list always re-renders.
 */
function loadVaults(): VaultStatus[] {
  return [...(props.vaults ?? mockApi.listVaults().vaults)];
}

/** The rendered list — reactive so `refreshAll()` and the Storybook control both flow through. */
const vaults = ref<VaultStatus[]>(loadVaults());

/** Keep Storybook controls reactive: when the `vaults` arg changes, reload. */
watch(
  () => props.vaults,
  () => {
    vaults.value = loadVaults();
  },
);

/** AC-4: the refresh-all affordance — re-invoke the seam (mock-driven re-fetch). */
...
<template>
  <section class="db">
    ...
    >
      <button class="db__add" type="button">+ Add a vault</button>
    </EmptyState>
