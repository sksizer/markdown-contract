...
export function createIpcTransport(): Transport {
  return {
    async check(vaultId: string): Promise<DriftResult> {
      return invoke("check", { vaultId });
    },
    async readConfig(vaultId: string): Promise<VaultConfig> {
      return invoke("read_config", { vaultId });
    },
    async saveConfig(vaultId: string, raw: string): Promise<null> {
      await invoke("save_config", { vaultId, raw });
      return null;
    },
    async listConfigFiles(vaultId: string): Promise<ConfigFiles> {
      return invoke("list_config_files", { vaultId });
    },
    async saveConfigFile(
      vaultId: string,
      relPath: string,
      ...
      await invoke("save_config_file", { vaultId, relPath, raw });
      return null;
    },
    async echo(message: string): Promise<string> {
