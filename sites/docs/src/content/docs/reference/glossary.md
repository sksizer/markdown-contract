...
| **mcVersion** | The schema-version envelope key on every declarative YAML document (currently `1`). It is reserved so a future `mcVersion: 2` could dispatch to new handling without breaking version-1 documents. See the [YAML reference](/reference/yaml/). |
