 ...
 *     → boot the localhost server (embedded SPA + JSON API), foreground,
 *       Ctrl-C to stop
 *
...
// daemon out from under us. See the README's entry-guard section.
import { runCli } from "markdown-contract/cli/run";

import { DEFAULT_PORT, hasUi, serve } from "./daemon";
...
function runDaemon(argv: string[]): void {
  ...
  const server = serve({ port, open: values.open === true });
