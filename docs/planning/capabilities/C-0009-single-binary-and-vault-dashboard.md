...
- It expands the product's *reach*, not its engine: the npm library stays canonical and untouched ([[PR-0001-markdown-contract]]); the binary and UI are additive build targets over the same `runner` ([[C-0003-corpus-cli]]). The shape is fixed by [[D-0010-distribution-single-exec-and-web-ui]].

## Statement

A user downloads a single file for their OS/arch and runs it. As a **CLI** it validates a tree exactly as the npm bin does. Run as **`markdown-contract daemon`**, the same binary starts a localhost web UI — served from assets embedded in the executable — that tracks a set of registered vaults and reports each one's validation status, refreshed on demand (or live, when watching). It lowers "use markdown-contract" from "install Node, then `npm i -g`" to "download one file", and lifts it from one-shot CLI runs to a standing, at-a-glance view across many vaults.
...
- **Stable JSON API** as the seam — the same API a future **Tauri** desktop wrap or an OS-native UI would consume (D-0010 § D6/D8).
- **Layered status** — registry (durable) / live status (in-memory) / history (optional SQLite) — so persistence can be added later without reshaping the UI (D-0010 § D4).

## Underlying implementation

- A single executable produced by **Bun `build --compile`** over the same sources, with the **Nuxt SPA (`ssr: false`) embedded** and served by a Nitro JSON API in `daemon` mode. The repo becomes a **Bun workspace orchestrated by moon** (`packages/core` + `apps/web`), adopted via [[T-MOON-adopt-moon-monorepo]].
- A new `daemon` consumer obeys the one-way layering (`daemon → runner/declarative`, never `core` internals), like the CLI.
- The runtime, UI shape, vault-tracking data model, channels, and the one packaging spike (embedding the SPA in the binary) are fixed by [[D-0010-distribution-single-exec-and-web-ui]]. Not yet built.
...
- Deliberately out of scope: multi-user/hosted service, auth, a Rust core, and a native (non-web) UI — the last two are recorded as future directions in D-0010 § D8.
