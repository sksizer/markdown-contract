...
The positional `<path>` is a **directory** (the run root), **not** a glob: `--contract <file>` runs that one contract against **every markdown file in the subtree below `<path>`** (the synthesized rule is `include: ['**/*.md']`). Output is selected with `--format human|json|sarif` (`human` is the console default), and the findings and CI-meaningful exit code are exactly the corpus CLI's ([[C-0003-corpus-cli]]). Internally the CLI assembles a one-rule `CorpusConfig` and runs the same `runCorpus`, so the result is identical to a meta-config carrying a single catch-all rule.

**Scope which files** with repeatable glob flags, relative to the run root: `--glob <glob>` (the include alias), `--include <glob>`, and `--exclude <glob>` — e.g. `validate ./docs --contract x.contract.yaml --glob 'releases/**/*.md' --exclude '**/_*.md'`. They narrow *within* the run root (they don't move it), and work the same here and in `--config` runs ([[C-0007-declarative-corpus-meta-config]]). Applying *different* contracts per subdirectory still needs a meta-config or the inline contract/target pairs ([[C-0007-declarative-corpus-meta-config]]).
