...
- The site **publishes to a public URL via Cloudflare Pages**: the `markdown-contract-docs` project, deployed by building locally and pushing the output (`moon run docs:build` → `wrangler pages deploy sites/docs/dist`). Live at <https://markdown-contract-docs.pages.dev/>. *(Supersedes the GitHub Pages plan: [[T-PAGE-docs-pages-deploy]] authored that workflow, but the repo is private so GitHub Pages can't be enabled; the workflow file has been removed.)*
...
- **Publishing moved to Cloudflare Pages (2026-07-03).** The GitHub Pages route died when the repo stayed private — Pages can't be enabled, so [[T-PAGE-docs-pages-deploy]]'s workflow could never publish (its only run failed at `configure-pages`). The site now publishes as the Cloudflare Pages project `markdown-contract-docs` (<https://markdown-contract-docs.pages.dev/>) via direct upload: `moon run docs:build`, then `wrangler pages deploy sites/docs/dist --project-name markdown-contract-docs`. Deploys are manual pushes — there is no git integration yet; wiring auto-deploy is a possible follow-up. T-PAGE stays closed: its authored deliverable shipped and was then superseded, and the dead workflow file is removed.
