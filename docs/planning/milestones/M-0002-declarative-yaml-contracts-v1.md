...
- The **CLI parameterization** for YAML, all compiling to one `CorpusConfig`: `validate <path> --contract <file>` (one contract → a tree, no meta-config), `validate <path> --config <meta.yaml>` (meta-config routing), and inline `--contract` / `--path` pairs.
...
3. **Meta-config loader + CLI wiring** ([[C-0007-declarative-corpus-meta-config]]). `loadConfig(yaml) → CorpusConfig`; the three contract-ref forms (registry name, `.yaml` path, inline definition); first-match traversal. CLI parameterization: `.yaml` / `.yml` recognition for `--config` beside `.js` / `.mjs`, the `--contract` flag (one contract → a tree), and repeated `--contract` / `--path` pairs (inline routing) — all assembling one `CorpusConfig`. Code-authored (`.ts` / `.js`) contract refs are deferred with the code escape.
