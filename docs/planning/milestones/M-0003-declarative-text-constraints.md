...
Decomposed into the task set below (minted as `T-*` docs):

1. **Text-match predicate core + `text/*` finding area** ([[T-TXMC-text-match-core]]). The pure matcher and the finding area / plane / id-synthesis scheme everything else emits through.
2. **TS-API predicate builders** ([[T-TXAP-text-predicate-builders]]). `requires` / `forbids` / `textRule` over the core, binding specs to section / document scope, enforcing requires-vs-forbids purity and per-entry id default/override.
3. **Declarative front-end** ([[T-TXYL-declarative-requires-forbids]]). Recognize `requires` / `forbids` on section nodes and the body root, validate the closed vocabulary, compile to the builders, and enforce compile-time consistency.
4. **Fixture corpus + dogfood** ([[T-TXFX-text-constraint-fixtures]]). YAML/TS parity fixtures, a dogfooded contract, and vitest discovery.
