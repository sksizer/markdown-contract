...
- [[quality-runner-streams-past-maxbuffer]] (https://github.com/sksizer/dev/pull/600) — linked to pre-existing upstream sdlc PR: quality-runner streams captured output past maxBuffer (not newly created; local dedup can't see the upstream corpus)
- [[task-work-step7-threads-baseline-dir]] (https://github.com/sksizer/dev/pull/598) — linked to pre-existing upstream sdlc PR: task-work Step 7 threads the main-repo baseline dir into worktree gate runs (not newly created)
- [[T-EW8J-refresh-planning-paths-post-monorepo-split]] — linked to existing local task: one-pass refresh of stale packages/core paths across the planning corpus
- [[T-LF7V-gap-report-strip-path-annotation]] (https://github.com/sksizer/dev/pull/609) — spawned upstream sdlc follow-up: gap-report strips a trailing parenthetical annotation from a Today path cell
- [[T-OX98-typed-row-source-coordinate-handle]] (https://github.com/sksizer/markdown-contract/pull/186) — spawned local follow-up: give model rows a typed source-coordinate handle so inlineSpans resolves by identity
