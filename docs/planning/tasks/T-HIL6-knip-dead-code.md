...
- `entry` lists the real entrypoints knip cannot infer because the manifest
  points at `dist/`: the three published source barrels (`src/index.ts`,
  `src/declarative/index.ts`, `src/cli/index.ts`), the test config
  (`vitest.config.ts`), the co-located unit tests (`src/**/*.test.ts`), and the
  integration corpus (`tests/**/*.test.ts`, `tests/harness.ts`,
  `tests/components.ts`, `tests/fixtures/**/*.ts`). Marking the published barrels
...
- `project` is `src/**/*.ts` — the set knip scans for unused source files and
...
| `moon.yml` | modify | Add a cached `lint-deps` task wrapping `npm run lint:deps`, `runInCI: false`. |
...
- [ ] AC-1: `knip.json` exists declaring `entry` (including `src/index.ts`, `src/declarative/index.ts`, `src/cli/index.ts`, `vitest.config.ts`, `src/**/*.test.ts`, and the `tests/` corpus) and `project` `src/**/*.ts`. A knip run reports **none** of the three published barrels' public exports, **no** co-located `*.test.ts` file, and **no** `tests/fixtures/**` file as unused — the barrel and test-corpus conventions produce zero false positives.
