...
> **Clarification (M-0005 planning, 2026-06-29).** "npm canonical" ([[D-0006-packaging]]) governs the *published artifact* (Node ESM via `tsc → dist`), **not** the dev-time package manager. The dev layer is **Bun**: `bun install` / `bun.lock` is the canonical lockfile and `package-lock.json` is removed. `build`/`typecheck` run on Bun for speed; the `test` task is pinned to **Node** so `vitest` proves the library runs on Node (the compat gate). T-MOON's interim npm-canonical setup (`package-lock.json`, `npm ci`, tasks wrapping `npm run`) is superseded by [[T-WKSP-bun-workspace-split]].
