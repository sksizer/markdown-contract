...
- **Two distinct resolution bases.** A relative **contract-file path** (in the `contracts` registry or written directly on a rule) resolves relative to **the config file's own directory** — config and contracts travel together. The rules' **`include` / `exclude` globs** instead match each file's path relative to **the run root** (the validated `<path>`, default cwd) — so one config can be aimed at different subtrees. Contract paths follow the config file; globs follow the run target.
