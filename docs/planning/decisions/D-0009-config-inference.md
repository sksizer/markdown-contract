...
- **Heading key-collisions — merge variants, fail clearly when unmergeable (T-KCOL).** Sections are keyed by their generated camelCase key (`core/camel.ts`), so two *distinct* spellings that collapse to the same key (e.g. `Schedule For today` / `Schedule For Today`) cannot be two sibling sections — the engine's build-time `contract/key-collision` guard would reject the contract. Inference therefore groups observed spellings by key and collapses each clash into **one** aliased section (first-seen spelling primary, the rest `aliases:`), with a warning naming the spellings, the shared key, and an example file per spelling. The one case that cannot be merged is two clashing spellings appearing **together as peers in a single doc** (the merged slot would match twice → `structure/duplicate-section`): that is a fatal, file-naming error that aborts the run with every clash reported at once — refusing to emit a contract that would fail its own self-check. (A heading with no alphanumerics yields no key and so never collides.) Letting a heading legitimately *recur* as peers, surfaced as a collection, is a separate future capability, not this guard.
...
A **`null`** observed value is handled *before* the rungs — no base rung admits `null`, so the field is inferred over its non-null values and the result marked `nullable` (an all-null field becomes a nullable string placeholder). Without this a null-valued field (e.g. `parent_key: null`) would infer `{ type: string }` and reject its own corpus.
