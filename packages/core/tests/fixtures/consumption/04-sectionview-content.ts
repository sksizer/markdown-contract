...
const c04 = defineConsumptionFixture({
  id: "c04",
  ...
  reads: [
    ...
    },
    {
      ...
      get: (doc) => doc.body.Summary.table,
      equals: undefined,
    },
    {
      label: "s.tables === [] — TableView[]",
      get: (doc) => doc.body.Summary.tables,
      equals: [],
    },
    {
      label: "s.lists === [] — ListView[]",
      get: (doc) => doc.body.Summary.lists,
      equals: [],
    },
    {
      label: "doc.body.why === undefined — absent optional section",
      // `why` is not a declared heading — the dynamic alias surface resolves it to undefined.
      get: (doc) => group(doc.body).why,
      equals: undefined,
    },
    {
      label: "doc.body.why?.text() === undefined — optional chaining tell",
      get: (doc) => asSection(group(doc.body).why)?.text(),
      equals: undefined,
    },
  ],
});
