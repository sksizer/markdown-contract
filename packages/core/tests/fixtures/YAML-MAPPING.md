...
- Translation follows the D-0020 mapping: the closed schema vocabulary (`type` / `enum` / `const` / `minLength`–`maxLength` / `minimum`–`maximum` / `minItems`–`maxItems` / `pattern` / `format` / `items` / `properties` / `required` / `additionalProperties` / `default`) ⇐ Zod; the body grammar (`sections` / `section` / `aliases` / `oneOf` / `minContains` / `maxContains` / `gap` / `anchor`, with nested levels hoisted onto the node); and the content leaves (`table` / `list` / `code` / `maxWords`). Table `cells` and frontmatter fields that use `z.enum` / `z.string().regex()` / `z.literal()` map straight into the closed vocabulary — they are **not** gaps.
- A feature that cannot reduce to that vocabulary — `.refine()` / `.superRefine()` / `z.unknown()`, or any `rule()` / `docRule()` — is dropped and marked `# GAP:` in the peer (no `$ref` or `rules:` is invented; both are outside the v2 subset).
- Every peer parses as YAML and carries `mcVersion: 2` / `kind: contract`.
