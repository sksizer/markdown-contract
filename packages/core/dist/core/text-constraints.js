 ...
 */
function assertRequiresPurity(specs) {
    for (const spec of specs) {
        const floor = Math.max(spec.min ?? 1, 1);
        if (spec.max !== undefined && spec.max < floor) {
            throw new ContractBuildError("contract/text-requires-purity", `requires(${specRepr(spec)}): a requires entry may not express absence ` +
 ...
 */
function sectionScopeText(node) {
    const buf = new LineBuffer();
    const walk = (n) => {
        for (const block of n.blocks)
            placeBlock(buf, block);
        for (const child of n.sections)
            walk(child);
    };
    walk(node);
    return buf.render();
 ...
 */
export function requires(specs) {
...
//# sourceMappingURL=text-constraints.js.map