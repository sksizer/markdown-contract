 ...
 */
export function formatFinding(f, opts) {
 ...
 */
export function filterFindings(findings, sel) {
    const idSet = sel.ids !== undefined ? new Set(sel.ids) : undefined;
    const { area } = sel;
    return findings.filter((f) => {
        if (area !== undefined && !(f.id === area || f.id.startsWith(area + "/")))
            return false;
        if (idSet !== undefined && !idSet.has(f.id))
            return false;
        return true;
    });
}
...
//# sourceMappingURL=finding-view.js.map