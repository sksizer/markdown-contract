...
function listView(node) {
    const items = node.items.map((raw, i) => {
...
function sectionGroup(nodes, seq) {
    ...
    const declaredKeyed = [];
    const repeatAt = new Map(); // exact heading name → index into declaredKeyed
    const unknown = [];
    ...
    for (const node of nodes) {
        ...
        const element = promotedTable(view, opts) ?? view;
 ...
 */
function bindDeclaredElement(declaredKeyed, repeatAt, name, element, repeatable) {
    if (!repeatable) {
    ...
    if (at === undefined) {
        ...
        declaredKeyed[at].value.push(element);
    }
}
/**
 ...
 */
function tableForRow(nodes, row) {
    for (const node of nodes) {
        for (const b of node.blocks) {
            if (b.kind !== "table")
                continue;
...
function rowIndexOf(node, target) {
