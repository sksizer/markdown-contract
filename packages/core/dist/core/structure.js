...
function unwrap(spec) {
    if (spec.kind === "optional") {
        const u = unwrap(spec.spec);
...
function makeSlot(specIdx, names, optional, opts) {
    return {
        specIdx,
        names,
...
function slotForName(slots, name) {
...
function isRepeatableName(slots, name) {
 ...
 */
function checkDuplicateHeadings(nodes, slots, ctx, out) {
    const seenName = new Map();
    const seenKey = new Map(); // camelKey → first heading text
 ...
 */
function checkRepeatBounds(nodes, slots, ctx, out) {
...
function emitRepeatBound(slot, nodes, ctx, out) {
...
function checkAliasAmbiguity(assigned, slots, ctx, out) {
    const slotBoundBy = new Map(); // slotIdx → the heading that first filled it
    for (const a of assigned) {
        if (a.slotIdx === null)
            continue;
        const slot = slots[a.slotIdx];
        if (slot.repeatable)
            continue; // a repeatable slot admits every spelling of its peers (T-1TA2)
        if (slot.names.length <= 1)
            continue; // single-spelling slot: handled by duplicate-section above
...
function checkMissingSlots(assigned, slots, nodes, ctx, out) {
    const filledSlots = new Set();
    for (const a of assigned)
        if (a.slotIdx !== null)
            filledSlots.add(a.slotIdx);
 ...
 */
function checkOrderAndUnknowns(order, allowUnknown, nodes, specs, slots, assigned, ctx, out) {
...
function emitStrayUnknowns(assigned, ctx, out) {
    for (const a of assigned) {
        if (a.slotIdx === null) {
            out.push(ctx.finding({
...
function runPresentSlotChecks(assigned, slots, ctx, out) {
    for (const a of assigned) {
        if (a.slotIdx === null)
            continue;
        const slot = slots[a.slotIdx];
...
function slotForOrNull(slots, name) {
 ...
 */
function checkRecognizedRelative(assigned, ctx, out) {
    let maxSlot = -1;
    for (const a of assigned) {
        if (a.slotIdx === null)
            continue;
...
function checkStrict(nodes, specs, slots, allowUnknown, ctx, out) {
    // Per-spec gap windows: specIdx → { min, max, count, anchorPos }.
    const gapCount = new Map();
    specs.forEach((s, i) => {
        if (unwrap(s).inner.kind === "gap")
            gapCount.set(i, 0);
    });
    ...
    const slotFill = new Map();
    /** Does `name` match a section/oneOf slot whose specIdx is ≥ `fromSpecIdx`? */
    function matchesSlotFrom(name, fromSpecIdx) {
        for (const slot of slots) {
            if (slot.specIdx >= fromSpecIdx && slot.names.includes(name))
                return slot.specIdx;
        }
        return null;
    }
 ...
 * the jumper.
 */
function admitPastSequence(node, specs, slots, allowUnknown, gapCount, ctx, out) {
...
function emitGapCounts(gapCount, specs, slots, nodes, ctx, out) {
    for (const [gapSpecIdx, count] of gapCount) {
        const gap = unwrap(specs[gapSpecIdx]).inner;
        const min = gap.min;
        const max = gap.max;
        if ((min !== undefined && count < min) || (max !== undefined && count > max)) {
            out.push(ctx.finding({
                id: "structure/gap-count",
                message: min !== undefined && count < min
 ...
 */
function checkUnorderedGap(assigned, specs, ctx, out) {
    ...
    if ((gap.min !== undefined && count < gap.min) || (gap.max !== undefined && count > gap.max)) {
        out.push(ctx.finding({
            id: "structure/gap-count",
            message: gap.min !== undefined && count < gap.min
...
function gapAnchorPos(specs, slots, nodes, gapSpecIdx) {
...
function runSectionChecks(node, opts, ctx, out) {
...
function anchorResolves(node, id) {
    if (node.anchors.includes(id))
        return true;
 ...
 */
function kindGate(node, leaf, anchor, ctx, out) {
    const expected = leaf.kind;
 ...
 */
export function matchStructure(tree, body, ctx) {
    const out = [];
 ...
 */
export function scanHeadingDepthJumps(root, ctx) {
    const out = [];
    const walk = (parent, isRoot) => {
