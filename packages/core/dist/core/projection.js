 ...
 */
function collectInlineSpans(nodes, source, out) {
    for (const n of nodes) {
 ...
 */
function inlineRaw(source, position, value) {
 ...
 */
function cellContentPos(cell) {
    const first = cell.children[0];
    if (first?.position)
        return pointPos(first.position.start);
    return posOf(cell);
}
...
function projectParagraph(node, source) {
...
function projectList(node) {
    const items = node.children.map((li) => {
        const item = { text: flattenListItem(li), pos: posOf(li) };
 ...
 * source line.
 */
function projectTable(node, source) {
    ...
    const rows = dataRows.map((row) => row.children.map((cell) => flattenInline(cell.children).trim()));
    ...
    const cellPositions = dataRows.map((row) => row.children.map(cellContentPos));
    const cellSpans = dataRows.map((row) => row.children.map((cell) => inlineSpansOf(cell.children, source)));
 ...
 */
function projectBlock(node, source) {
    switch (node.type) {
        case "paragraph":
...
function newSection(name, depth, pos) {
 ...
 */
function buildTree(root, source) {
    ...
    for (const node of root.children) {
        if (node.type === "yaml")
            continue; // frontmatter handled separately
...
function attachSection(stack, child) {
    ...
    stack[stack.length - 1].sections.push(child);
    stack.push(child);
}
/**
 ...
 */
function attachContentNode(sec, node, source) {
 ...
 */
function bindTrailingAnchor(block) {
...
function buildFrontmatter(node) {
    ...
    function lineForPath(path) {
        if (path.length === 0)
            return blockBaseLine + 1; // the block itself → first key line
        const localOffset = keyOffset(doc.contents, path);
 ...
 */
function keyOffset(contents, path) {
    let node = contents;
    let offset;
 ...
 */
export function parse(markdown, opts) {
    ...
    const yamlNode = mdast.children.find((c) => c.type === "yaml");
