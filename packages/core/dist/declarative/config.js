...
export function loadConfig(yamlText, baseDir) {
...
function compileConfig(raw, baseDir) {
...
function compileRule(rule, path, contracts, baseDir) {
    if (!isMap(rule))
        throw new DeclarativeError(`${path}: a rule must be a mapping`);
...
function resolveContract(ref, path, contracts, baseDir) {
    if (ref === undefined)
        throw new DeclarativeError(`${path}: a rule needs a contract (a name, a .yaml path, or an inline contract)`);
    if (isMap(ref)) {
    ...
    const target = typeof contracts[ref] === "string" ? contracts[ref] : ref;
    if (!/\.ya?ml$/i.test(target)) {
        throw new DeclarativeError(`${path}: in v1 a contract ref must be a .yaml file (got '${target}'); referencing a code-authored .js/.ts contract is the deferred code escape (D-0008)`);
    }
