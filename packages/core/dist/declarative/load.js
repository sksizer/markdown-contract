...
export function loadContract(yamlText) {
...
export function compileContractObject(raw) {
    const def = {};
    if (raw.frontmatter !== undefined) {
        def.frontmatter = compileFrontmatter(raw.frontmatter);
    }
    if (raw.body !== undefined) {
        def.body = compileBody(raw.body);
        ...
        if (raw.body !== null && typeof raw.body === "object" && !Array.isArray(raw.body)) {
            const docRule = compileBodyTextRule(raw.body);
            // biome-ignore lint/suspicious/noAssignInExpressions: idiomatic default-and-push
            if (docRule)
                (def.rules ??= []).push(docRule);
        }
    }
    return contract(def);
}
function compileFrontmatter(fm) {
