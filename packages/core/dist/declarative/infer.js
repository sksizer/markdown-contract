...
function discover(root, scope) {
    const include = scope?.include && scope.include.length > 0 ? picomatch(scope.include, PICOMATCH_OPTS) : null;
    const exclude = scope?.exclude && scope.exclude.length > 0 ? picomatch(scope.exclude, PICOMATCH_OPTS) : null;
...
function asFrontmatterMap(data) {
 ...
 */
function allIdenticalSequences(docs) {
    if (docs.length === 0)
        return false;
    const first = docs[0].sections;
    return docs.every((d) => d.sections.length === first.length && d.sections.every((n, i) => n === first[i]));
}
/**
...
function hasOrderConflict(edges) {
 ...
 */
function topoSort(union, edges) {
    const out = [];
    const emitted = new Set();
    const predsSatisfied = (name) => {
        for (const [from, tos] of edges) {
            if (tos.has(name) && !emitted.has(from))
                return false;
        }
        return true;
    };
...
function allMatchFormat(values, format) {
 ...
 */
function inferFieldSchema(values, fileCount, opts) {
    if (values.length === 0)
        return { type: "string" };
    ...
    if (values.every((v) => typeof v === "number")) {
        return values.every((n) => Number.isInteger(n))
    ...
    if (values.every((v) => typeof v === "string")) {
        return stringRung(values, fileCount, opts);
    }
 ...
 */
function constRung(values, opts) {
 ...
 */
function stringRung(strings, fileCount, opts) {
    for (const format of FORMAT_CANDIDATES) {
        if (allMatchFormat(strings, format))
 ...
 */
function enumRung(strings, fileCount, opts) {
 ...
 */
function inferFrontmatter(docs, opts) {
    const keys = [];
    const seen = new Set();
...
function dedupePreservingOrder(names) {
    const seen = new Set();
    const out = [];
 ...
 */
function inferBody(docs, relax, sink) {
    ...
    const repeatedSpellings = new Set();
    for (const doc of docs) {
        ...
        for (const [name, n] of counts)
            if (n >= 2)
                repeatedSpellings.add(name);
    }
    const byKey = groupSpellingsByKey(sections);
    const entries = [];
    const emitted = new Set(); // primary spellings already emitted
...
function groupSpellingsByKey(sections) {
    const byKey = new Map();
    for (const name of sections) {
        const key = toCamelKey(name);
        if (key === "")
            continue; // no alphanumerics ⇒ no generated alias ⇒ cannot collide
 ...
 */
function emitSectionEntry(name, byKey, emitted, repeatedSpellings, docs, sink, entries) {
    const key = toCamelKey(name);
    const spellings = key === "" ? [name] : byKey.get(key);
    const primary = spellings[0];
    if (name !== primary)
        return; // an alias spelling — its slot is emitted at the primary
    if (emitted.has(primary))
        return;
    emitted.add(primary);
    ...
    if (spellings.some((s) => repeatedSpellings.has(s)))
        entry.repeatable = true;
    entries.push(entry);
    if (aliases.length > 0)
        recordAliasMerge(spellings, key, primary, docs, sink);
}
/**
 ...
 */
function recordAliasMerge(spellings, key, primary, docs, sink) {
    const filesWith = (spelling) => docs.filter((d) => d.sections.includes(spelling)).map((d) => d.rel);
    const quoted = spellings.map((s) => `‘${s}’`).join(" / ");
    const coDocs = docs
        .filter((d) => spellings.filter((s) => d.sections.includes(s)).length > 1)
        .map((d) => d.rel);
    if (coDocs.length > 0) {
        sink.errors.push(`sibling headings ${quoted} collapse to the same key ‘${key}’ and appear together as peers ` +
        ...
        sink.warnings.push(`merged variant headings ${quoted} into one section ‘${primary}’ (shared key ‘${key}’); seen as ${examples}`);
    }
}
// ── Naming & emission ──────────────────────────────────────────────────────────────
/**
 ...
 */
function nameForDir(relDir, absRoot) {
    if (relDir === "") {
 ...
 */
function generalize(docs, opts, sink) {
    const def = {};
 ...
 */
function inferMeta(absRoot, docs, depth, opts, inline, sink) {
    // Route each doc to a group key (its depth-`depth` ancestor dir), tracking stranded files.
    const groups = new Map(); // group dir → its docs (first-appearance order)
    const groupOrder = [];
    const stranded = [];
    let hasRoot = false;
    for (const doc of docs) {
    ...
    const contracts = orderedKeys.map((key) => ({
        name: nameForDir(key, absRoot),
        include: [key === "" ? "*.md" : `${key}/**/*.md`],
        def: generalize(groups.get(key), opts, sink),
    }));
    for (const rel of stranded) {
        sink.warnings.push(`stranded: ${rel} sits above the --depth ${depth} cut and is covered by no contract; ` +
 ...
 */
function emitMetaFiles(contracts, inline) {
    if (inline) {
        const config = {
            mcVersion: 1,
            kind: "config",
    ...
    for (const c of contracts)
        registry[c.name] = `./contracts/${c.name}.contract.yaml`;
    const config = {
        mcVersion: 1,
        kind: "config",
        contracts: registry,
 ...
 */
export function inferConfig(root, opts) {
    ...
    const docs = discover(absRoot, { include: opts?.include, exclude: opts?.exclude }).map((rel) => parseDoc(absRoot, rel));
    ...
    if (opts?.meta === true && depth >= 1) {
        ...
        const contract = { name, include: ["**/*.md"], def };
    ...
    if (sink.errors.length > 0) {
        const lead = sink.errors.length === 1
